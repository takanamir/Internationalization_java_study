...
public class PropertyFileSample {
	public static void main(String[] args) throws IOException {
		...
		try (Reader reader = new InputStreamReader(new FileInputStream(
				"src/readProperty/sample.properties"), StandardCharsets.UTF_8)) {
			// 入力ストリームからロード
		...
		try (Writer writer = new OutputStreamWriter(new FileOutputStream(
				"src/readProperty/sample2.properties"), StandardCharsets.UTF_8)) {
			properties.store(writer, // ファイルに書き込みを行なうためのWriter
